...
export default class GitHubGenerator extends BaseGenerator {
	constructor(...args) {
		...
		this.option('labels', { defaults: true, type: Boolean });
	}
	configuring() {
	...
	writing() {
		...
		return this._spawn('git', ['remote', 'add', 'origin', 'git@github.com:' + this.options.user + '/' + this.options.name + '.git']);
	}
	end() {
		return Promise.all([
			this._spawn('git', ['push']),
			this.options.labels && Promise.all([
